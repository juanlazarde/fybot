...
def save_profile(
    profile_name: str, params: dict, profiles: dict
) -> Union[Tuple[bool, None, dict], bool]:
...
if "wtc_latest" in st.session_state:
    if len(st.session_state["wtc_latest"]) > 0:
        params["WATCHLIST"]["watchlist_current"] = st.session_state["wtc_latest"]
        params["WATCHLIST"]["selected"] = list(params["WATCHLIST"].keys()).index(
            "watchlist_current"
        )

_watchlist = [
    ", ".join(Watchlist.sanitize(params["WATCHLIST"][k]))
...
with st.sidebar.expander(label="Save profile", expanded=False):
    with st.form(key="profile_name", clear_on_submit=True):
        new_profile_name = st.text_input(
            label="Profile Name", help="Name of the new profile. Keep it short"
        )
    ...
    if submitted:
        ...
        if saved:
            ...
            st.error(
                f"Profile '*{new_profile_name}*' couldn't be saved,"
...
with st.sidebar.expander(label="Other settings", expanded=False):
    ...
    # TODO: This import is breaking the page  # Calculator
            ...
            if params["DEBUG"]["export"]:
                for k in nested_df:
                    if nested_df[k] is not None:
                        export_results(strategy=k, df=nested_df[k], path=ss.DATASET_DIR)
