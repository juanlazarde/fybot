...
# def app():
st.title("News")
sources = st.sidebar.selectbox(
    label="Select your sources", options=ss.NEWS["sources"], index=0
)
st.header(sources)
subsources = st.sidebar.selectbox(
    label="Select your subs", options=ss.NEWS["subsources"], index=0
)
...
with st.spinner(text="Loading the news..."):
    ...
    counts, mentions, rows = nw.show_reddit(subsources=subsources, num_days=num_days)
