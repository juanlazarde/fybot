...
DATABASE_FILE = os.path.join(S.DATASET_DIR, 'app.db')
...
class Download:
    ...
    def symbols_alpaca(self):
        ...
        # tradable_assets = {asset
        #                    for asset in active_assets if asset.tradable}
        # tradable_assets = [(asset.symbol, asset.name)
        #                     for asset in tradable_assets]
        tradable_assets = [(asset.symbol, asset.name)
                           for asset in active_assets if asset.tradable]
        result = [dict(zip(['symbol', 'name'], asset))
                  for asset in tradable_assets]
    ...
    def symbols_sp500_wikipedia(self):
        ...
        result = df.to_dict('records')[:S.MAX_SYMBOLS]
        self.sp500 = result
        return result

    def yahoo_tickers(self, source='sp500'):
        assets = self.symbols_sp500_wikipedia() \
            if source == 'sp500' \
            else DataBase().fetch_database()
